...
export default function Discussion({ productId }: DiscussionProps) {
  ...
  const { data: discussions, isLoading, isError } = useQuery<DiscussionType[]>({
    ...
    queryFn: async () => {
      const response = await fetch(`/backend/product/${productId}/discussions`, {
        cache: 'no-store'
      });
  ...
  return (
    <section id="discussions" className="w-full py-6">
      ...
      <div className="grid lg:grid-cols-[1fr_380px] gap-4 w-full lg:px-4 py-4 lg:py-6">
        <div className="max-md:order-last w-full md:border-r border-border border-dashed max-sm:border-t">
          {
            isLoading ?
              <Loader /> :
              (isError || !discussions) ?
                <div className="relative z-10 flex h-36 w-full items-center justify-center rounded-full bg-background/30 p-2">
                  <p className="text-muted-foreground">Failed to fetch product discussion&#39;s.</p>
                </div> :
